...
<body>
  ...
  <div style="border-right : 3px solid lightgray"class="col-3">
    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
      ...
      <a class="nav-link" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-selected="false">Relatório </a>
      <a class="nav-link" id="v-pills-messages-tab" data-toggle="pill" href="#v-pills-messages" role="tab" aria-controls="v-pills-messages" aria-selected="false">Dados</a>
      <a class="nav-link" id="v-pills-settings-tab" data-toggle="pill" href="#v-pills-settings" role="tab" aria-controls="v-pills-settings" aria-selected="false">Configurações</a>
        <?php echo("<a class='nav-link' id='v-pills-settings-tab' data-toggle='pill' role='tab' aria-controls='v-pills-settings' aria-selected='false'> <a href='removerl.php?cod=$codCursoTT'>&nbsp &nbsp  Logout</a></a>")?>
    </div>
  </div>
  <div class="col-9">
    <div class="tab-content" id="v-pills-tabContent">
      <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
       <div class="table-responsive-md">
          <div id='busca'>
              <form action="curso.php" class="search" method="get">
      ...
      <tbody>
        <?php
           $codCursoTT = $_GET["cod"];
            ...
            if(isset($_GET['pesq'])){
                $busca = $_GET['pesq'];
                $sqli = mysqli_query($conn, "SELECT * FROM tbCursoaluno WHERE nome LIKE '$busca' AND codCursoconfirmado= '$codCursoTT' ");
                while ($linha = mysqli_fetch_array($sqli)){
                    $codAluno = $linha['codAluno'];
                    echo("<tr>
                        <th scope='row'>{$linha['codAluno']}</th>
                        <td>{$linha['nomeAluno']}</td>
                        <td>{$linha['emailAluno']}</td>
                        <td>{$linha['cpfAluno']}</td>
                        <td>{$linha['rgAluno']}</td>
                        <td>{$linha['enderecoAluno']}</td>
                       
                    
                    </tr>");
                }
            } else {
                $sqli = mysqli_query($conn, "SELECT * FROM tbcursoAluno WHERE codCursoconfirmado= '$codCursoTT'");
               while ($linha = mysqli_fetch_array($sqli)){
                    $codAluno = $linha['codAluno'];
                    echo("<tr>
                        <th scope='row'>{$linha['codAluno']}</th>
    ...
    <div class="tab-pane fade" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
         ...
         <div class="grafico" >
          ...
          <form id="form1" name="form1" method="get" action="exibepdf.php">
